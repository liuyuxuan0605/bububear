...
void* Block_Epoll_Net::recv_task(void* arg)
{
    ...
    do
    {
        //长度头也可能短读（read 返回 1~3 字节），必须循环读满 4 字节，否则后续全部错位
        int nHeadOffset = 0;
        while (nHeadOffset < (int)sizeof(nPackSize))
        {
            nRelReadNum = read(ev->fd, (char*)&nPackSize + nHeadOffset, sizeof(nPackSize) - nHeadOffset);
            if (nRelReadNum <= 0)
                break;
            nHeadOffset += nRelReadNum;
        }
        if (nHeadOffset != (int)sizeof(nPackSize))
            break;

        //校验包大小：<=0（畸形）或超上限（恶意/错位）都按断连处理，
        //防止 new char[负数/2G] 直接把服务端打挂
        if (nPackSize <= 0 || nPackSize > _DEF_MAX_PACK_SIZE)
        {
            printf("recv_task: illegal nPackSize=%d, fd=%d, close\n", nPackSize, ev->fd);
            break;
        }
...
// 借助 do..while 在最后统一处理错误  避免使用goto语句
    ev->eventdel();              // 从 epoll 监听树摘掉这个 socket
    close(ev->fd);               // 关掉连接

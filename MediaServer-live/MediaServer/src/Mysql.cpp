...
int CMysql::SelectMysql(char* szSql,int nColumn,list<string>& lst)
{
    ...
    while((record = mysql_fetch_row(results)))  // 遍历查询结果
    {
        ...
        {
            // record[i] 可能为 NULL（如某列允许空），直接 push_back(NULL) 构造 std::string 会崩，先兜底
            lst.push_back( record[i] ? record[i] : "" );  // 将查询结果存入list
        }
    }
    mysql_free_result(results);   // 必须释放结果集！否则后续 mysql_query 会报 "Commands out of sync" 而失败
